---
title: "Statistical Method Final Exam"
...
In this regression model, \( \beta_1 \) quantifies the estimated effect of **Weight** on **Blood Fat**, controlling for **Age**. The estimate \( \beta_1 = 0.42 \) indicates that for each 1-unit increase in weight, blood fat levels are expected to increase by **0.42 units**, assuming age remains constant.  However, this effect is **not statistically significant** (*p* = 0.573), and the 95% confidence interval \( [-1.09,\ 1.93] \) contains zero. This implies that the association between weight and blood fat may be due to random variation, and there is **insufficient evidence** to conclude that weight is a significant predictor in this model.
...
## 1.8 1.8.	Find an estimate for the expected blood fat level if the weight of the person is 88 kg and the age of the same person is  is 58 years.
...
```{r message=FALSE, warning=FALSE}
full_mlr <- lm(Blood_Fat ~ Weight*Age, data = Weight)
...
p <- length(coef(mlr)) # includes intercept
n <- nrow(Weight)
...
```{r message=FALSE, warning=FALSE}
# DFFITS
dffits_vals <- dffits(mlr)
# DFBETAS
dfbetas_vals <- dfbetas(mlr)
# Cook’s Distance
...
The current multiple linear regression model, which includes Weight and Age as predictors of Blood Fat, explains approximately **70.56%** of the variability in the response variable. The model shows a statistically significant improvement when Age is added **(F = 47.457, p < 0.001)**, indicating that both predictors contribute meaningfully to the explanation of Blood_Fat levels. There is no strong evidence of multicollinearity if VIF values remain below critical thresholds. 


# Problem 2

## 2.1) The first-order response function to be used in the study is:

$Y = \beta_0 + \beta_1x_1 + \beta_1x_1 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \epsilon$


## 2.2) Develop the response function for each type of lamp.

$Y_{Led} = \beta_0 + \beta_1x + \beta_2 = (\beta_0 + \beta_2) + \beta_1x$

$Y_{Incandescent} = \beta_0 + \beta_1x + \beta_3 = (\beta_0 + \beta_3) + \beta_1x$

$Y_{Halogen} = \beta_0 + \beta_1x + \beta_4 = (\beta_0 + \beta_4) + \beta_1x$

$Y_{Fluorescent} = \beta_0 + \beta_1x$ 


## 2.3) Specify the null and alternative hypotheses for the following question. With the humidity level fixed, is the survival time of the LED bulb more than the survival time for incandescent bulb?

$H_0: \beta_2 \le \beta_3$

$H_1: \beta_2 > \beta_3$


## 2.4) Interpret the value $\beta_3$ and what does it represent.

$\beta_3$ represents the difference in mean survival time between incandescent bulbs and fluorescent bulbs, after controlling for humidity.


# Problem 3

## 3.1) Write an expression for $Y = \beta_0 + \beta_1x_1 + \epsilon_i, i = 1,..,4$  using matrices and vectors
...
## 3.2) Set up and solve the b vector.

```{r message=FALSE, warning=FALSE}
# Define X matrix (with intercept column)
X <- matrix(c(1, 1, 1, 1,
              -3, -2, 5, -1), 
            nrow = 4, byrow = FALSE)

# Define Y vector
Y <- matrix(c(7, 11, 3, 10), nrow = 4, byrow = FALSE)
...
## 3.3) Calculate the Hat matrix H
...
```{r message=FALSE, warning=FALSE}
model <- arima(diff_timedata, order = c(0,1,1))
