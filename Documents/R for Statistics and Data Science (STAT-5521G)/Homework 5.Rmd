...
## (Grad student only) In addition to the for loop solution, create a solution using only componentwise calculations. For a bonus, create a solution using `apply()`.
```{r}
...
paste("The total service cost using `apply()` is", total_cost_apply)
```
